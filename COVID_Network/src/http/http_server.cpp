...
//        printf("http:selctNode\n");
    ...
    else if(route_check(http_req,"/sortCoreness")){
        char id[10];
        mg_http_get_var(&http_req->query,"id",id,sizeof(id));
        int index = charToInt(id);
        sortCoreness(connection,index);
    }
}
...
void HttpServer::testRandomAttack(mg_connection *connection,int index,int divide){
    if(index >= tests.size()){
        printf("testRandomAttack：参数有误！\n");
        json j;
        j["error"] = std::string("testRandomAttack：参数有误！\n");
    ...
    std::vector<int> ids;
    Test test = tests[index];
    ids = test.testRandomAttack(divide,Rs,ASPs,MCSRs);
    json j;
...
void HttpServer::testIntentialAttack(mg_connection *connection,int index,int divide){
    if(index >= tests.size()){
        printf("testIntentialAttack：参数有误！\n");
        json j;
        j["error"] = std::string("testIntentialAttack：参数有误！\n");
    ...
    std::vector<int> ids;
    Test test = tests[index];
    ids = test.testIntentialAttack(divide,Rs,ASPs,MCSRs);
    json j;
    test.attackResultToJson(j,ids);
    for(int i=0;i<Rs.size();i++){
        j[i]["ratio"]=Rs[i];
...
void HttpServer::sortCoreness(mg_connection *connection,int index){
    if(index >= tests.size()){
        printf("countPaperByYear：参数有误！\n");
        json j;
        j["error"] = std::string("countPaperByYear：参数有误！\n");
